...
pair<Mat, Rect> RobustTextDetection::apply( Mat& image ) {
	...
	for (vector<ComponentProperty>::const_iterator iter=props.begin();iter!=props.end();++iter)
	{
		if( (*iter).area < param.minConnCompArea || (*iter).area > param.maxConnCompArea )
			continue;

		if( (*iter).eccentricity < param.minEccentricity || (*iter).eccentricity > param.maxEccentricity )
			continue;

		if( (*iter).solidity < param.minSolidity ||(*iter).solidity>param.maxSolidity)
			continue;

		result |= (labels == (*iter).labelID);
	}
	...
	for (vector<ComponentProperty>::const_iterator iter=props.begin();iter!=props.end();++iter)
	{
		...
		double accum = 0.0;
		for (vector<int>::const_iterator int_iter=nonzero_vec.begin();int_iter!=nonzero_vec.end();++int_iter)
		{
			int val=*int_iter;
	...
	if(!bounding_region_coord.data)
	{
		cout<<"find no NonZero area!"<<endl;

	}
	//namedWindow("NonZero",1);
	//imshow("NonZero",bounding_region_coord);
	//waitKey();
...
Mat RobustTextDetection::computeStrokeWidth( Mat& dist ) {
    ...
    int max_stroke = static_cast<int>(cvRound( max_val_double ));
    ...
    for( int stroke = max_stroke; stroke > 0; stroke-- ) {
		...
		for (vector<Point>::const_iterator iter=stroke_indices.begin();iter!=stroke_indices.end();++iter)
		{
			Point stroke_index=*iter;
        ...
        while( !neighbors.empty() ){
			for(vector<Point>::const_iterator iter=neighbors.begin();iter!=neighbors.end();++iter)
			{
				Point neighbor=*iter;
			...
			for (vector<Point>::const_iterator point_iter=temp.begin();point_iter!=temp.end();++point_iter)
			{
				Point neighbor=*point_iter;
